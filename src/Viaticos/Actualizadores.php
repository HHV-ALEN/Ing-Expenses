...
// Actualiza el estado a 'Fuera de Rango' 
// Obten todos los registros en estado ' Prórroga'

$SQL = "SELECT * FROM viaticos WHERE Estado = 'Prórroga'";
$Result = mysqli_query($conn, $SQL);

if (mysqli_num_rows($Result) > 0) {
    while ($Row = mysqli_fetch_assoc($Result)) {
        $Id = $Row['Id'];
        $Fecha_Prorroga = $Row['Fecha_Prórroga'];

        // Imprimir fecha de regreso para depuración
        //echo "<br>Fecha de Prorroga: " . $Fecha_Prorroga;

        // Suma 3 días hábiles a la fecha de regreso
        $fechaLimite = sumarDiasHabiles($Fecha_Prorroga, 3);
