...
// Consulta SQL con LEFT JOIN para verificar si existen evidencias
$sql_query = "SELECT viaticos.*, usuarios.Nombre AS nombre_usuario, 
              (CASE 
                WHEN EXISTS (SELECT 1 FROM evidencias WHERE evidencias.Id_Viatico = viaticos.Id) 
                THEN 1 
                ELSE 0 
               END) AS tiene_evidencias
              FROM viaticos
              INNER JOIN usuarios ON viaticos.Id_Usuario = usuarios.Id 
              WHERE viaticos.Estado IN ('EnEspera', 'Verificación', 'EnCurso', 'Comprobación', 'Segunda Revisión','FueraDeRango') 
              ORDER BY viaticos.Fecha_Solicitud DESC 
              LIMIT $offset, $records_per_page";

$result = $conn->query($sql_query);

// Consulta para contar el total de registros filtrados por estado
$total_records_query = "SELECT COUNT(*) AS total FROM viaticos WHERE Estado IN ('EnEspera', 'Verificación', 'EnCurso', 'Comprobación', 'Segunda Revisión','FueraDeRango')";
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        // Clase condicional: 'table-info' si tiene evidencias
                        $row_class = $row['tiene_evidencias'] == 1 ? 'table-info' : '';
