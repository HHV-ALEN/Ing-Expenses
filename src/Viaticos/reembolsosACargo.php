...
$sql_query = "SELECT reembolso.*, usuarios.Nombre AS Nombre_Usuario 
              FROM reembolso 
              JOIN usuarios ON reembolso.Id_Usuario = usuarios.Id 
              WHERE reembolso.Id_Gerente = '$id_user' AND reembolso.Id_Viatico = 0
              ORDER BY reembolso.Id DESC 
              LIMIT $offset, $records_per_page";
$result = $conn->query($sql_query);

// Consulta para contar el total de registros
$total_records_query = "SELECT COUNT(*) AS total FROM reembolso WHERE Id_Gerente = '$id_user' AND reembolso.Id_Viatico = 0";
...
<head>
    ...
    <title>Listado de Reembolsos a mi cargo</title>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        <th style="text-align:center">Folio de la solicitud(ID)</th>
                                        <th style="text-align:center">Folio del reembolso(ID)</th>
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<td style='text-align:center'><a href='../Control/detallesReembolso.php?id_reembolso=" . $row['Id'] . "' class='btn btn-outline-primary'>Ver Información</a></td>";
                                            echo "</tr>";
                                        }
                                    } else {
                                        echo "<tr><td colspan='8' style='text-align:center'>No hay viáticos registrados.</td></tr>";
                                    }
                                    ?>
                                </tbody>
                            </table>
